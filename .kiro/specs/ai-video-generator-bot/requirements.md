...
Система автоматической генерации рекламных видеороликов представляет собой Flask веб-приложение, развернутое на Render.com, которое взаимодействует с пользователями через Telegram бота. Система принимает текстовое или голосовое описание темы рекламного ролика и автоматически генерирует готовое 50-секундное видео с озвучкой, используя OpenAI API для создания сценария, изображений, анимаций и аудио, а также FFmpeg для сборки финального видео.

**Примечание:** Текущая версия оптимизирована для быстрой отладки (10 сегментов). После тестирования можно масштабировать до полной версии (48 сегментов, 4 минуты).
...
- **Final Video**: Готовый 50-секундный рекламный ролик с озвучкой (тестовая версия)
- **Script Assistant**: Специализированный OpenAI Assistant для генерации сценариев
- **Segment Assistant**: Специализированный OpenAI Assistant для разбивки сценария и создания промптов изображений
- **Animation Assistant**: Специализированный OpenAI Assistant для создания промптов анимации
...
**User Story:** Как пользователь, я хочу получить профессионально написанный сценарий для 50-секундного ролика, чтобы видео было структурированным и качественным

#### Критерии Приемки

1. THE Flask Application SHALL отправить описание темы в Script Assistant с предустановленным Assistant ID для генерации сценария
2. THE Script Assistant SHALL создать сценарий для 50-секундного видео с тремя частями: вступление (15 сек), основная часть (25 сек), заключение (10 сек)
3. THE Flask Application SHALL отправить сценарий в Segment Assistant для разделения на 10 сегментов по 5 секунд каждый
4. THE Segment Assistant SHALL разделить сценарий на сегменты: intro (0-2), main (3-7), outro (8-9)
5. THE Flask Application SHALL сохранить каждый сегмент сценария с временной меткой и порядковым номером
6. IF OpenAI Assistant возвращает ошибку, THEN THE Flask Application SHALL повторить запрос до 3 раз с экспоненциальной задержкой
...
1. THE Flask Application SHALL отправить каждый сегмент в Segment Assistant для генерации промпта изображения
2. THE Segment Assistant SHALL создать детальный промпт для изображения на основе текста сегмента
3. THE Flask Application SHALL отправить каждый промпт в Runway API для генерации изображения
4. THE Flask Application SHALL ожидать завершения генерации изображения через Runway API (polling или webhook)
5. THE Flask Application SHALL загрузить сгенерированное изображение и сохранить его локально с уникальным идентификатором
6. THE Flask Application SHALL связать каждое изображение с соответствующим Video Segment
7. WHILE генерация изображений выполняется, THE Telegram Bot SHALL отправлять пользователю обновления о прогрессе каждые 5 сегментов
8. IF генерация изображения через Runway API завершается ошибкой, THEN THE Flask Application SHALL повторить запрос до 2 раз перед уведомлением пользователя об ошибке
...
1. WHEN все изображения сгенерированы, THE Telegram Bot SHALL отправить пользователю галерею из первых 3 изображений как превью
...
1. THE Flask Application SHALL отправить каждый сегмент в Animation Assistant для генерации промпта анимации
2. THE Animation Assistant SHALL создать детальный промпт для анимации на основе текста сегмента и контекста
3. THE Flask Application SHALL отправить изображение и промпт анимации в Runway API для создания 5-секундного видео
4. THE Flask Application SHALL ожидать завершения анимации через Runway API (polling или webhook)
5. THE Flask Application SHALL загрузить анимированное видео и сохранить его локально с уникальным идентификатором
6. THE Flask Application SHALL связать каждое анимированное видео с соответствующим Video Segment
7. WHILE анимация выполняется, THE Telegram Bot SHALL отправлять пользователю обновления о прогрессе каждые 5 сегментов
8. IF анимация через Runway API завершается ошибкой, THEN THE Flask Application SHALL повторить запрос до 2 раз перед уведомлением пользователя об ошибке
...
1. WHEN все видео сегменты анимированы, THE Telegram Bot SHALL отправить пользователю первые 2 видео сегмента как превью
...
3. THE OpenAI TTS SHALL создать аудиофайл длительностью приблизительно 50 секунд
4. THE Flask Application SHALL загрузить и сохранить аудиофайл локально
5. THE Flask Application SHALL синхронизировать длительность аудио с общей длительностью видео (10 сегментов × 5 секунд = 50 секунд)
